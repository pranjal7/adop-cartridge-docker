...
getDockerfile.with {
 ...
 steps {
  shell(''
   'set +x | echo "Pull the Dockerfile out of Git, ready for us to test and if successful, release via the pipeline." |
   | #Convert tag name to lowercase letters
   if any uppercase letters are present since they are not allowed by Docker | echo TAG = $(echo "$IMAGE_TAG" | awk '{print tolower($0)}') > build.properties |
   | #Export out credential ID to a properties file | echo LOGIN = $(echo $ {
    DOCKER_LOGIN
   }) >> build.properties | set - x ''
   '.stripMargin())
   environmentVariables {
  ...
  parameters {
   ...
   steps {
    copyArtifacts('Get_Dockerfile') {
     ...
     shell(''
      'set +x | echo "We are printing the Docker File" | cat Dockerfile | set - x ''
      ').stripMargin
     }
    }
    publishers {
   ...
   staticCodeAnalysis.with {
     ...
     steps {
      ...
      shell(''
        'set +x | echo "Mount the Dockerfile into a container that will run Dockerlint: https://github.com/RedCoolBeans/dockerlint" | docker run--rm - v jenkins_slave_home: /jenkins_slave_home/--entrypoint = "dockerlint"
        redcoolbeans / dockerlint - f / jenkins_slave_home / $JOB_NAME / Dockerfile > $ {
         WORKSPACE
        }
        /${JOB_NAME##*/
       }.out |
       |
       if !grep "Dockerfile is OK"
      $ {
       WORKSPACE
      }
      /${JOB_NAME##*/
     ...
     then
      | echo "Dockerfile does not satisfy Dockerlint static code analysis" | cat $ {
       WORKSPACE
      }
     /${JOB_NAME##*/
    }.out | exit 1 |
   ...
   | fi |
   | echo "LOGIN=${DOCKER_LOGIN}" > credential.properties | set - x ''
  '.stripMargin())
  environmentVariables {
...
dockerBuild.with {
 ...
 steps {
  ...
  shell(''
   'set -x | echo "Building the docker image locally..." | docker build - t $ {
    DOCKERHUB_USERNAME
   }
   /${IMAGE_TAG}:${B} ${WORKSPACE}/. |
   | echo LOGIN = $(echo $ {
    DOCKER_LOGIN
   }) > credential.properties | set + x ''
   '.stripMargin())
   environmentVariables {
    propertiesFile('credential.properties')
   }
 ...
 vulnerabilityScan.with {
   ...
   steps {
    ...
    shell(''
     'set +x | echo "THIS STEP NEEDS TO BE UPDATED ONCE ACCESS TO A PRODUCTION CLAIR DATABASE IS AVAILABLE" |
     |
     if [-z $ {
      CLAIR_DB
     }]; then | echo "WARNING: You have not provided the endpoints for a Clair database, moving on for now..." |
     else
      |#Set up Clair as a docker container | echo "Clair database endpoint: ${CLAIR_DB}" | mkdir / tmp / clair_config | curl - L https: //raw.githubusercontent.com/coreos/clair/master/config.example.yaml -o /tmp/clair_config/config.yaml
      | #Add the URI
     for your postgres database | sed - i '' - e "s|options: |options: ${CLAIR_DB}|g" / tmp / clair_config / config.yaml | docker run - d - p 6060 - 6061: 6060 - 6061 - v / tmp / clair_config: /config quay.io/coreos / clair - config = /config/config.yaml | #INSERT STEPS HERE TO RUN VULNERABILITY ANALYSIS ON IMAGE USING CLAIR API | fi |
     | echo LOGIN = $(echo $ {
      DOCKER_LOGIN
     }) > credential.properties | set - x ''
     '.stripMargin())
   ...
   imageTest.with {
     ...
     steps {
      ...
      shell(''
        'set -x | echo "Use the darrenajackson/image-inspector container to inspect the image" | #Set test file directory |
        ...
        export docker_workspace_dir = $(echo $ {
         WORKSPACE
        } | sed 's#/workspace#/var/lib/docker/volumes/jenkins_slave_home/_data#') |
        ...
        var / run / docker.sock: /var/run / docker.sock darrenajackson / image - inspector - i $ {
         DOCKERHUB_USERNAME
        }
        /${IMAGE_TAG}:${B} -f ${TEST_DIR}/image.cfg > $ {
         WORKSPACE
        }
        /${JOB_NAME##*/
       }.out |
     ...
     then
      | echo "Your built image has failed testing..." | cat $ {
       WORKSPACE
      }
     /${JOB_NAME##*/
    }.out | exit 1 |
...
containerTest.with {
 ...
 steps {
  ...
  shell(''
    'set -x | echo "Building a new test image installing required applications, running test suite and destroying the new image and container at the end of the tests..." | #Set test file directory |
    ...
    for test dockerfile generation | source $ {
     WORKSPACE
    }
    /${TESTS_PATH}/dockerfile / dockerfile_envs.sh |
    ...
    /*
                ...
                |docker exec ${IMAGE_TAG}-${IMG_EXT} ${TEST_DIR}/container_tests.sh ${TEST_DIR} > ${WORKSPACE}/${JOB_NAME##*/
   }.out |
   | #Stop and clean up the testing container and testing image | docker stop $ {
    IMAGE_TAG
   } - $ {
    IMG_EXT
   } && docker rm - v $ {
 ...
 then
  ...
  if grep - E "expected port closed|expected process incorrect owner|expected process incorrect owner"
 $ {
  WORKSPACE
 }
 /${JOB_NAME##*/
...
then
 | echo "Your container has failed testing..." | exit 1 | fi | echo "Some unexpected ports/processes found, moving on for now..." |
 else
  |cat $ {
   WORKSPACE
  }
/${JOB_NAME##*/
 ...
 | echo "Container has successfully passed testing..." | fi |
 | echo LOGIN = $(echo $ {
  DOCKER_LOGIN
 }) > credential.properties | set - x ''
'.stripMargin())
...
dockerPush.with {
  ...
  steps {
   shell(''
    'set +x | docker login - u $ {
     DOCKERHUB_USERNAME
    } - p $ {
     DOCKERHUB_PASSWORD
    } - e devops @adop.com | docker push $ {
     DOCKERHUB_USERNAME
    }
    /${IMAGE_TAG}:${B} |
    | echo LOGIN = $(echo $ {
  ...
  dockerDeploy.with {
    ...
    steps {
     shell(''
      'set +x | docker login - u $ {
       ...
       DOCKERHUB_PASSWORD
      } - e devops @adop.com | docker run - d--name jenkins_$ {
       IMAGE_TAG
      }
      _$ {
       B
    ...
    dockerCleanup.with {
      ...
      steps {
       shell(''
        'set +x | #Checking to see whether to delete all containers or just one |
        if [$ {
         CONTAINER_DELETION
        } = "SINGLE"]; then | echo "Deleting single container..." | docker rm - f jenkins_$ {
         IMAGE_TAG
        }
        _$ {
         B
         ...
         CONTAINER_DELETION
        } = "ALL"]; then | echo "Deleting all containers..." |
        for i in `seq 1 ${B}`; | do
         |
         if docker
        ps - a | grep "jenkins_${IMAGE_TAG}_${i}"; then | docker rm - f jenkins_$ {
         IMAGE_TAG
        }
        _$ {
         i
